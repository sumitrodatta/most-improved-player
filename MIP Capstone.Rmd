...
Theoretically, it is possible to have the most first-place votes and not win the award, but it has never happened in the history of the award. MIP Vote Share is tracked for both the season in which the vote was received, as well as the season prior. The latter column will help identify candidates for next season. For example, Pascal Siakam of the Toronto Raptors won the MIP award in 2019 with an MIP vote share of 0.938. This would be recorded in Siakam's 2019 season under `MIP Share Current Season` and in Siakam's 2018 Season under `MIP Share Next Season`.
...
```{r mip_winner_shares, echo=FALSE, attr.output='style="max-height: 200px;"'}
...
```{r mip_ppg, fig.align='center',echo=FALSE}
...
The lowest points per game average by an MIP is 0.06 (achieved by Rony Seikaly in 1990), while the highest is 0.22 (achieved by Ryan Anderson in 2012).
...
```{r mip_winner_share_over_years, echo=FALSE, fig.align='center'}
...
```
Personal fouls per game seems like an odd choice for most important variable, but it's much likely correlated with minutes played per game.
...
```
The 5 most important variables match up with expectations. The first two are advanced holistic statistics, while the next three measure offensive improvement, which is easier to recognize than defensive improvement.
...
models_2021 %>% knitr::kable(align = 'c')
```
...
```{r decision_tree2021, echo=FALSE}
...
```
The top 5 most important variables in the ranger model all relate to scoring.
...
Lister is the first candidate we've seen that received extra opportunity. While he increased his per game statistics due to being given 8 more minutes of playing time per game, his rate statistics dropped.

McGee is 32, and the oldest player to have won MIP was Darrell Armstrong (who was 30). Hernangomez has spent enough time sitting on the bench in the past few seasons that it's safe to say there won't be any improvement: he is who he is. Nance was a popular offseason pick on the condition that his team (the Cleveland Cavaliers) alleviate the logjam at his position so he could start and play big minutes. That did not happen.

One curious component about the Ranger model is that the predicted big men (Lister, McGee, Nance and Hernangomez) had a significantly older average age than the guards (28 vs 23).
```{r rborist2021}
...
models_2021 %>% knitr::kable(align = 'c')
```
The Rborist RMSE is similar to the linear RMSE. Willis improved across the board in 1987. Although his minutes per game only increased by 4, he attempted more shots and converted them at a higher rate (despite conventional wisdom stating that as shot volume go up, shooting percentages go down). Willis had sound defensive impact, but he improved more on the offensive end.
...
knitr::kable(list(winner2020[1:5,],winner2020[6:10,]),align = "c")
knitr::kable(list(winner2020[11:15,],winner2020[16:20,]),align = "c")
...
knitr::kable(list(winner2021[1:5,],winner2021[6:10,]),align = "c")
knitr::kable(list(winner2021[11:15,],winner2021[16:20,]),align = "c")
knitr::kable(list(winner2021[21:25,],winner2021[26:30,]),align = "c")
```
There's a more eclectic mix of candidates (superstars like Stephen Curry and James Harden were each predicted by one model), and the vote shares are much lower than the 2020 winner predictions. This was expected, since there is far more variance in player seasons before receiving MIP votes. Two players might have very similar statistics in one season, but one of the players received more playing time in the subsequent season and received MIP votes. The algorithm would be confused and split down the middle.

Allen, Zubac and Anunoby all exemplify the young player asked to fill a certain role on championship contending teams. They are not asked to shoulder the scoring load, but are expected to focus on defense and contribute when the ball swings to them on offense. Melton is an advanced statistics gem, but will need to increase his points per game next year in order to get writers talking about his candidacy. 
...
* Jaren Jackson (3 out of 5): highly touted franchise cornerstone for the Memphis Grizzlies
...
By nature, the per game & per 36 minutes & per 100 possessions are very correlated. I didn't remove highly correlated variables by design, as I wanted the models themselves to perform feature selection and determine what the most important variables were.
